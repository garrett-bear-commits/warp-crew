...
- Mandatory player text is at least 16 CSS px, instruction text 18 CSS px, touch targets at least 44×44 CSS px. One primary action per state, concrete language, safe areas, reduced-motion parity. A guided spotlight may dim irrelevant UI but must elevate the real tappable target and not hide the watched ship/combat event; only one primary button pulses. Sparse red dots mark actionable new/ready containers and clear on use, never locked/paid/disabled content.
