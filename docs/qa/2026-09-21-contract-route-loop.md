...
Fresh production build: 51 transformed modules; `dist/index.html` 1.27 kB (0.75 kB gzip), `index-Dzw4hJeH.css` 30.67 kB (7.16 kB gzip), and `index-CGhskIeP.js` 251.19 kB (81.02 kB gzip). Browser: Chrome 153.0.8010.53, desktop headless mobile viewport emulation.
...
Each viewport records 16 normal-motion component states, 36 actionable-control samples, and 161 essential text samples, plus the reduced-motion order state. Disabled controls are measured too. All measured required controls reach at least 44×44 CSS pixels; every card/consequence text sample is at least 16px. The harness scrolls each target into view, checks its whole rectangle against the viewport and every clipping ancestor, and verifies center-point hit ownership below the navigation boundary. It resets both scroll axes before overview captures.
...
- Contrast and the wider pre-existing UI are not comprehensively audited here. The 44px/16px results apply to the named package components and sampled states.
